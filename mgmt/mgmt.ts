...
export function parsePermission(permission: string | string[] | number) {
	if (typeof permission === "number") {
		if (Number.isInteger(permission) && permission >= 0) return permission;
		throw new Error(
			"Permission must be a non-negative integer or known permission flags.",
		);
	}
	...
	const normalized = (
		...
		.trim()
		.toLowerCase();
	if (!normalized) throw new Error("Permission cannot be empty.");
	if (/^\d+$/.test(normalized)) return Number(normalized);
	if (normalized === "all" || normalized === "full") return 63;

	const parts = normalized
		.split(/[\s,+|]+/)
		.filter(Boolean)
		.flatMap(part =>
			permissionFlags[part] || /^\d+$/.test(part)
				? [
						part,
					]
				: part.split(""),
	...
	const value = parts.reduce((sum, part) => {
		if (/^\d+$/.test(part)) return sum | Number(part);
		const flag = permissionFlags[part];
		if (!flag) throw new Error(`Unknown permission flag "${part}".`);
		return sum | flag;
	}, 0);
...
export async function resolveUserId(idOrLogin: string) {
	...
	if (!user) throw new Error(`User "${idOrLogin}" not found.`);
	return user._id;
}

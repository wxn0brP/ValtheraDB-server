...
Example:
    ...
    if (isNaN(access)) {
        access = 0;
        if (accessRaw.includes("f"))
            access += 1; // "f" for find
        if (accessRaw.includes("a"))
            access += 2; // "a" for add
        if (accessRaw.includes("r"))
            access += 4; // "r" for remove
        if (accessRaw.includes("u"))
            access += 8; // "u" for update
        if (accessRaw.includes("c"))
            access += 16; // "c" for collection
        if (accessRaw.includes("n"))
            access += 32; // "n" for unknown
    }
